/**
*
* Carousel
*
*/

import React from 'react';
import PropTypes from 'prop-types';
import sortBy from 'lodash.sortby';

import { SliderTrack } from '../SliderTrack';
import { SlideWrapper } from '../SlideWrapper';
import { RightArrow as DefaultRightArrow } from '../RightArrow';
import { LeftArrow as DefaultLeftArrow } from '../LeftArrow';
import { SliderWrapper } from '../SliderWrapper';
import { SliderList } from '../SliderList';
import { PaginationDotsWrapper as DefaultDotsWrapper } from '../PaginationDotsWrapper';
import { PaginationDot as DefaultDot } from '../PaginationDot';
import Timer from './Timer';


class Carousel extends React.Component { // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
    this.state = {
      initialCard: 0,
      childWidth: 0,
      cardsToShow: 0,
      hideArrows: false,
    };
  }

  componentDidMount() {
    const {
      children, cardsToShow: cardsToShowProp,
      autoSlide, hideArrowsOnNoSlides,
    } = this.props;
    const childrenCount = children ? children.length || 1 : 0;
    const cardsToShow = cardsToShowProp || childrenCount;
    const childWidth = 100 / cardsToShow;
    this.setState({ // eslint-disable-line react/no-did-mount-set-state
      childWidth,
      cardsToShow,
      hideArrows: hideArrowsOnNoSlides && childrenCount <= cardsToShow,
    }, () => this.updateResponsiveParams());
    typeof window !== 'undefined' && // eslint-disable-line no-unused-expressions
      window.addEventListener('resize', this.updateResponsiveParams);
    if (autoSlide) {
      this.autoSlider = new Timer(() => {
        let updatedInitialCard = 0;
        if (childrenCount - this.state.cardsToShow > this.state.initialCard) {
          updatedInitialCard = this.state.initialCard + 1;
        }
        this.setState({
          initialCard: updatedInitialCard,
        });
      }, (autoSlide === true ? 2000 : autoSlide));
      this.autoSlider.start();
    }
  }

  componentWillUnmount() {
    typeof window !== 'undefined' && // eslint-disable-line no-unused-expressions
      window.removeEventListener('resize', this.updateResponsiveParams);
  }

  updateResponsiveParams = () => {
    const { children, hideArrowsOnNoSlides } = this.props;
    let { breakpoints } = this.props;
    const childrenCount = children ? children.length || 1 : 0;
    if (breakpoints) {
      breakpoints = sortBy(breakpoints, 'breakPoint');
      let updatedCardsToShow = this.state.cardsToShow;
      breakpoints.forEach(({ breakPoint, cardsToShow }) => {
        if (breakPoint <= window.innerWidth) {
          updatedCardsToShow = cardsToShow;
        }
      });
      const updatedInitialCard = (childrenCount - updatedCardsToShow) < this.state.initialCard ? (childrenCount - updatedCardsToShow) : this.state.initialCard;
      this.setState({
        cardsToShow: updatedCardsToShow,
        childWidth: 100 / updatedCardsToShow,
        initialCard: updatedInitialCard,
        hideArrows: hideArrowsOnNoSlides && childrenCount <= updatedCardsToShow,
      });
    }
  }

  changeInitialCard = (initialCard) => {
    const { afterSlide, beforeSlide } = this.props;
    if (beforeSlide) {
      beforeSlide();
    }
    this.setState({
      initialCard,
    }, () => {
      if (afterSlide) {
        afterSlide();
      }
    });
  }

  handleLeftArrowClick = (evt) => {
    const { children } = this.props;
    const { cardsToShow } = this.state;
    const childrenCount = children ? children.length : 0;
    if (evt && evt.preventDefault) { evt.preventDefault(); }
    let nextInitialCard = this.state.initialCard - 1;
    if (nextInitialCard < 0) {
      nextInitialCard = childrenCount - cardsToShow;
    }
    this.changeInitialCard(nextInitialCard);
  }

  handleRightArrowClick = (evt) => {
    const { children } = this.props;
    const { cardsToShow } = this.state;
    const childrenCount = children ? children.length : 0;
    if (evt && evt.preventDefault) { evt.preventDefault(); }
    let nextInitialCard = this.state.initialCard + 1;
    if (childrenCount - cardsToShow < nextInitialCard) {
      nextInitialCard = 0;
    }
    this.changeInitialCard(nextInitialCard);
  }

  renderChildren = (children) => {
    const { childWidth } = this.state;
    const displayCards = [];
    React.Children.forEach(children, (child, index) => {
      displayCards.push((
        <SlideWrapper key={index} width={childWidth}>
          {child}
        </SlideWrapper>
      ));
    });
    return displayCards;
  }

  renderDots = () => {
    const dots = [];
    const { children, Dot } = this.props;
    const childrenCount = children ? children.length || 1 : 0;
    let i;
    for (i = 0; i <= childrenCount - this.state.cardsToShow; i += 1) {
      const index = i;
      dots.push(React.cloneElement(Dot, {
        active: index === this.state.initialCard,
        key: index,
        onClick: () => this.changeInitialCard(index),
      }));
    }
    return dots;
  }

  renderLeftArrow = () => {
    const { LeftArrow, infinite } = this.props;
    const { initialCard } = this.state;
    return React.cloneElement(LeftArrow, {
      onClick: this.handleLeftArrowClick,
      disabled: !infinite && !initialCard,
    });
  }

  renderRightArrow = () => {
    const { RightArrow, children, infinite } = this.props;
    const childrenCount = children ? children.length || 1 : 0;
    const { initialCard, cardsToShow } = this.state;
    return React.cloneElement(RightArrow, {
      onClick: this.handleRightArrowClick,
      disabled: !infinite && (initialCard + cardsToShow === childrenCount),
    });
  }

  render() {
    const {
      children,
      cardsToShow,
      showDots,
      showArrows,
      pauseOnMouseOver,
      DotsWrapper,
      ...otherProps
    } = this.props;
    const { initialCard, childWidth } = this.state;
    return (
      <div
        onMouseLeave={() => pauseOnMouseOver && this.autoSlider && this.autoSlider.resume()}
        onMouseEnter={() => pauseOnMouseOver && this.autoSlider && this.autoSlider.pause()}
      >
        <SliderWrapper {...otherProps}>
          {showArrows && !this.state.hideArrows && this.renderLeftArrow()}
          <SliderTrack>
            <SliderList translateX={initialCard * childWidth}>
              {this.renderChildren(children, cardsToShow || children.length)}
            </SliderList>
          </SliderTrack>
          {showArrows && !this.state.hideArrows && this.renderRightArrow()}
        </SliderWrapper>
        <DotsWrapper>
          {showDots && this.renderDots()}
        </DotsWrapper>
      </div>
    );
  }
}

Carousel.defaultProps = {
  showDots: true,
  showArrows: true,
  LeftArrow: <DefaultLeftArrow />,
  RightArrow: <DefaultRightArrow />,
  Dot: <DefaultDot />,
  DotsWrapper: DefaultDotsWrapper,
  cardsToShow: null,
  afterSlide: null,
  beforeSlide: null,
  infinite: true,
  breakpoints: null,
  autoSlide: 2000,
  pauseOnMouseOver: true,
  padding: '0px 20px',
  margin: '0px',
  hideArrowsOnNoSlides: true,
};

Carousel.propTypes = {
  LeftArrow: PropTypes.node,
  RightArrow: PropTypes.node,
  Dot: PropTypes.node,
  showArrows: PropTypes.bool,
  showDots: PropTypes.bool,
  children: PropTypes.node.isRequired,
  cardsToShow: PropTypes.number,
  afterSlide: PropTypes.func,
  beforeSlide: PropTypes.func,
  infinite: PropTypes.bool,
  breakpoints: PropTypes.arrayOf(PropTypes.shape({
    breakPoint: PropTypes.number,
    cardsToShow: PropTypes.number,
  })),
  autoSlide: PropTypes.oneOfType([
    PropTypes.number,
    PropTypes.bool,
  ]),
  pauseOnMouseOver: PropTypes.bool,
  padding: PropTypes.string,
  margin: PropTypes.string,
  hideArrowsOnNoSlides: PropTypes.bool,
  DotsWrapper: PropTypes.func,
};

export default Carousel;
